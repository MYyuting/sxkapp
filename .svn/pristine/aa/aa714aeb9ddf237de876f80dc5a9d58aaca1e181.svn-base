						...
						<div class="PaiList">
								<ul v-show="!flagWu">
										<li v-for="(item,index) in arrList">
												...
												<div>
														<div class="left-img">
																<img :src="item.userTeam.userAvatar" alt="" :onerror="defaultImg">
														</div>
														<div>
												...
												<button :class="item.userTeam.attentionStatus ==1 ? 'active ' : ''"
														...
														{{item.userTeam.attentionStatus == 1 ? '取消' : '关注'}}
												</button>
										</li>
								</ul>
    ...
    export default {
        name: "xgpaihang",
				...
				methods:{
						...
						//本页数据列表
						getData(){
								this.$axios.post(url+'/api/userStarlightDetails/findUserStarlightDetails',this.$qs.stringify({
										token:this.token,
