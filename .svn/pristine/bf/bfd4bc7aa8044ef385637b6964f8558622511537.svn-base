				...
				<!--<div class="quotat_banner">-->
						<!--<swiper :options="swiperOption" ref="Myswiper" class="quotat_swiper">-->
								...
								<!--<swiper-slide class="quotat_banner_slide">-->
										...
										<!--</div>-->
								<!--</swiper-slide>-->
								<!--&lt;!&ndash;<swiper-slide class="quotat_banner_slide">&ndash;&gt;-->
										<!--&lt;!&ndash;<div v-for="item in arr[2].arra" class="quotat_slide_item">&ndash;&gt;-->
												<!--&lt;!&ndash;<div><span>{{item.nam}}</span><span>{{item.num}}</span></div>&ndash;&gt;-->
												<!--&lt;!&ndash;<div>{{item.fen}}</div>&ndash;&gt;-->
										<!--&lt;!&ndash;</div>&ndash;&gt;-->
...
<script>
		...
		export default {
				...
				mounted: function () {
						//初始化首页
						this.$axios.post(url+'/api/investors/findTInvestorsTag').then((res) => {
								console.log(res.data)
								this.tabList = this.tabList.concat(res.data.tagList);
						});
								...
								setTimeout(() => {
										...
										switch (curNavIndex) {
												...
												case 2:
														url1=url+'/api/investors/findInvestorsList';
														var typeN = 2;
														var typeName = that.tabList[2];
														break;
												case 3:
														url1=url+'/api/investors/findInvestorsList';
														var typeN = 2;
														var typeName = that.tabList[3];
														break;
												case 4:
														url1=url+'/api/investors/findInvestorsList';
														var typeN = 2;
														var typeName = that.tabList[4];
														break;
										}
										this.$axios({
